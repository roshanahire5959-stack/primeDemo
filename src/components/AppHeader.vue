    ...
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
      ...
      <div class="flex-1 max-w-xl">
        <div class="relative">
          ...
          <input type="text" placeholder="Search TV shows…" v-model.trim="store.searchQuery"
            class="w-80 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl pl-9 pr-4 py-2 text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:border-indigo-500 focus:bg-white/10 transition-all cursor-pointer" />
        </div>
      </div>
      <nav class="hidden md:flex items-center gap-1">
        <RouterLink :to="{ name: 'home' }"
          ...
          Browse {{ store?.filteredVideos.length }}
        </RouterLink>
      </nav>
    </div>
  </header>
